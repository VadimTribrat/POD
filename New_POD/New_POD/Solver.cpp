...
std::vector<double> Solver::calculateBB(const std::vector<double>& params)
{
	...
	double C_11Deriv = derivative(E_0, params, { 1 + 1e-2, 1 + 1e-2, 1 }, { 1 - 1e-2, 1 - 1e-2, 1 });
	double C_12Deriv = derivative(E_0, params, { 1 + 1e-2, 1 - 1e-2, 1 }, { 1 - 1e-2, 1 + 1e-2, 1 });
	double BDeriv = derivative(E_0, params, { 1 + 1e-2, 1 + 1e-2, 1 + 1e-2 }, { 1 - 1e-2, 1 - 1e-2, 1 - 1e-2 });
	res.push_back(2 * (BDeriv / (9 * V_0)) * 0.8019);

	res.push_back(((C_11Deriv + C_12Deriv) / (2*V_0))*0.8019);
	res.push_back(((C_11Deriv - C_12Deriv) / (2 * V_0))*0.8019);


	double C_44Deriv = derivative(E_0, params, { 1, 1e-2, 1e-2, 1, 1 / (1 - 1e-2 * 1e-2) },
