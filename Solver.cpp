...
std::vector<double> Solver::Calculate(const std::vector<double>& params)
{
	...
	double C_11 = ((C_11_derivative + C_12_derivative) / (4 * V_0));
	double C_12 = ((C_11_derivative - C_12_derivative) / (4 * V_0));

	double B_derivative = Derivative2(E_0, params, { 1 + alpha, 1 + alpha, 1 + alpha }, { 1 - alpha, 1 - alpha, 1 - alpha });
	double B = (B_derivative / (9 * V_0));

	double C_44_derivative = Derivative2(E_0, params, { 1, alpha, alpha, 1, 1 / (1 - alpha * alpha) },
		{ 1, -alpha, -alpha, 1, 1 / (1 - alpha * alpha) });
	double C_44 = (C_44_derivative / (4 * V_0));

	//std::cout << "C_12_derivative = " << C_12_derivative << "\n";
	//std::cout << "C_11_derivative = " << C_11_derivative << "\n";
	//std::cout << "V_0 = " << V_0 << "\n";
	//std::cout << "C_12 = " << C_12 << "\n";
	//std::cout << "C_11 = " << C_11 << "\n\n";
	if (isnan(E_coh))
