...
Lattice Lattice::GenerateLattice(Atom::ElementType type, double multiplier) {
    std::vector<Atom> basis = {
            Atom(type, Point(0, 0, 0)),
            Atom(type, Point(0.5, 0, 0.5)),
            Atom(type, Point(0.5, 0.5, 0)),
            Atom(type, Point(0, 0.5, 0.5))
    };
    ...
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) {
            for (int k = 0; k < 3; ++k) {
                for (auto& atom : basis) {
                    //result.push_back(Atom(type, Point::multiply(Point(atom.pos + Point(i, j, k)),
                    //    Point(multiplier, multiplier, multiplier))));
                    result.push_back(Atom(type, Point(atom.pos + Point(i, j, k))));
                }
            }
        }
    }
    ...
    return Lattice(result, Point(3, 3, 3), multiplier);
}

Lattice::Lattice(const std::vector<Atom>& atoms, Point period, double multiplier)
